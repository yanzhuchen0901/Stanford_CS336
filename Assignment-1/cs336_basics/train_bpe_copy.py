...
def train_bpe(
    ...
    while len(vocab) < vocab_size:
        ...
        # 3. 更新受影响的单词 (完全保留原变量名 words, tokens, pair2word)
        ...
        for word_str in word_strs:
            ...
            # A. 移除旧 Pair (修正 pop 逻辑，防止 Counter 堆积)
            for i in range(len(tokens) - 1):
                old_pair = (tokens[i], tokens[i + 1])
                pair_counts[old_pair] -= count
                if pair_counts[old_pair] <= 0:
                    pair_counts.pop(old_pair, None) # 使用 pop 彻底从内存移除键
                
                # 更新逆向索引
                if old_pair in pair2word and word_str in pair2word[old_pair]:
                    pair2word[old_pair][word_str] -= 1
                    if pair2word[old_pair][word_str] <= 0:
                        del pair2word[old_pair][word_str]
                    if not pair2word[old_pair]:
                        pair2word.pop(old_pair, None)
            ...
            for i in range(len(new_tokens) - 1):
                ...
                if new_pair not in pair2word:
                    pair2word[new_pair] = Counter()
                pair2word[new_pair][word_str] += 1
        
        # 4. 清理当前已合并完的 pair 索引，释放内存
        pair2word.pop(most_common_pair, None)
        
        # 增加一个简单的进度打印，防止 500MB 运行过程中看起来像死机
        if len(vocab) % 100 == 0:
            print(f"Current vocab size: {len(vocab)}", end='\r')
